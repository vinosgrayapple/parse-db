...

{
  ...
  ######################################################### 
{
 gsub(/фірма Quinta dos Ingleses/,"Quinta dos Ingleses",INFO);
 gsub(/фірма Cantina Delibori Walter s.r.l./," Cantina Delibori Walter s.r.l.",INFO);
 gsub(/\s*ТОВ\s*/,"",INFO);
 gsub(/\s*ПИ[ИК]\s*/,"",INFO);
 gsub(/Виробник.*СП\s*/,"Виробник ",INFO);
  ...
  } else if (COUNTRY_TRADE ~ /0/) {
    COUNTRY_TRADE="Дип.Вантаж";
  } 
  ...
  PRODUCER = trim(str);
  if (PRODUCER ~ /ольский ?/) {
    PRODUCER="ЗАТ Тираспольский винно-коньячный завод KVINT";
  } else if (PRODUCER ~ /^ирту$/) {
    PRODUCER="";
  } else if (PRODUCER~/\s*ООО\s*/) {
    PRODUCER=EXPORTER;
  } else if (PRODUCER~/а/) {
     PRODUCER=EXPORTER;
  }
  ...
  if (PDO[UKTZED] != "") {
    ...
    match(INFO,/(вирощене в регiоні | вироблене в регіоні |вироблені регіонах |регіону cорту | регіону |з ?регіону |регіоні|вироблені у регіоні|вироблене в регіоні|регіонах|зонах|\bрегіону|\bзоні|\bзона|регіон|вироблене\ *в|PDO\)\,\ без\ додавання\ спирту\:.*\/)[\x3A\-\x3C ]*([A-Za-z][0-9A-Za-z\x27\x20\x2D\x2E\x26\x2F\x60]+)/,eng);
    match(INFO,/(вирощене в регiоні | вироблене в регіоні |вироблені регіонах |регіону cорту | регіону |з ?регіону |регіоні|вироблені у регіоні|вироблене в регіоні|регіонах|зонах|\bрегіону|\bзоні|\bзона|регіон|вироблене\ *в)[\x3A\-\x3C ]*([А-ЯЇІЄҐа-яіїґ\.\-\x27]+( [А-ЯЇІЄҐа-яіїґ\x27]+)?)/,ukr);
    indx = 2;
    if (eng[indx] !~ /^[\s\-\: ]*$/) {
      str=eng[indx];
    } else if (ukr[indx] !~ /^[\s\-\: ]*$/) {
  ...
  PDO_COUNTRY=trim(PDO_COUNTRY);
  gsub(/(виробництва |[\s(]*$)/,"",PDO_COUNTRY)
  ...
  gsub(/ /, "", QUANTITY);
  gsub(/\./, ",", QUANTITY);
  if (QUANTITY=="" && NETTO_KG==BRUTTO_KG) {
    QUANTITY=NETTO_KG;
  }
  if (QUANTITY=="" && NETTO_KG==BRUTTO_KG) {
    QUANTITY=NETTO_KG;
  }
  if (COUNTRY_TRADE==""&&COUNTRY_OF_ORIGIN!="") {
    COUNTRY_TRADE=COUNTRY_OF_ORIGIN;
  }
  ...
  if (PRODUCER~/\s*ООО\s*/) {
   ...
   gsub(/(ТОВАРИСТВО З ОБМЕЖЕНОЮ ВІДПОВІДАЛЬНІСТЬЮ|Товариство з обмеженною відповідальністю|Товариство з обмеженою відповідальністю|ТОВАРИСТВО З ОБМЕЖЕНОЮ ВIДПОВIДАЛЬНIСТЮ)/,"ТОВ ",IMPORTER);
   gsub(/Дочірнє підприємство/,"ДП ",IMPORTER);
   gsub(/(Приватне підприємство|ПРИВАТНЕ ПІДПРИЄМСТВО)/,"ПП ",IMPORTER);
   gsub(/Публічне акціонерне товариство/,"ПАТ ",IMPORTER);
   gsub(/ПРИВАТНЕ АКЦІОНЕРНЕ ТОВАРИСТВО/,"ПрАТ ",IMPORTER);
   gsub(/ТОРГОВИЙ ДІМ/," ТД ",IMPORTER);
   gsub(/^ТОВ/,"& ",IMPORTER);
   gsub(/(\x27|\.|\x2B)/," ",IMPORTER);
   gsub(/\s+/," ",IMPORTER);
   gsub(/немає?( ?данн?их)?/,"NO_TM",TM);
   if (INFO == "") {
     INFO = "NO_INFO";
   }
   if (CURRENCY == "") {
   ...
   if (IMPORTER == "") {
     IMPORTER="NO_IMPORTER";
   } else if (IMPORTER ~ /(Джангозін Мухіт|Адлан Ануар|Аташат з торгово-економічних питань Республіки Перу в Україні|Генеральне Консульство |EMBASSY|Посольство | Аташат)/) {
     IMPORTER="EMBASSY";
   }
}
